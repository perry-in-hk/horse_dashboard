...
export async function bootstrapInitialAdmin() {
  const { rows } = await pool.query("SELECT COUNT(*)::int AS c FROM dashboard_users");
  ...
  await pool.query(`INSERT INTO dashboard_users (username, password_hash, role) VALUES ($1, $2, 'admin')`, [
    username,
    passwordHash,
  ]);
